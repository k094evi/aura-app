// src/features/signin-signup/components/VerifyEmail.tsx
'use client';
...
interface VerifyEmailProps {
   ...
   *  Throw (or reject) with a message to surface it as the inline error text. */
  onVerify?: (code: string) => Promise<void> | void;
  /** Called when the user taps "Resend". Defaults to a no-op. */
  onResend?: () => Promise<void> | void;
  /** Where "Back to Sign Up" / "Back to Home" should send the user. */
  signUpHref?: string;
  homeHref?: string;
}
      ...
      <div className="z-10 flex flex-col items-center gap-5 px-4">
        ...
        <div className="flex w-full max-w-[480px] flex-col items-start gap-7 rounded-3xl border-[1.5px] border-white bg-white/70 px-6 py-9 shadow-[0px_4px_16px_0px_rgba(124,58,237,0.05),0px_20px_60px_0px_rgba(17,24,39,0.09)] backdrop-blur-md sm:px-10">
          <div className="flex w-full flex-col items-center gap-2 text-center">
            <h1 className="text-[28px] font-extrabold text-gray-900">Verify Your Email</h1>
          ...
          {error && <p className="w-full text-center text-[13px] leading-[18px] text-[#ef4444]">{error}</p>}
          ...
          <div className="flex w-full items-start justify-center">
            <p className="text-sm">
              <span className="text-gray-600">Want to try another email? </span>
              <Link href={signUpHref} className="font-semibold text-[#7c3aed]">
                Back to Sign Up
              </Link>
            </p>
          </div>
