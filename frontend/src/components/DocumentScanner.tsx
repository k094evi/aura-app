...
export default function DocumentScanner() {
  return (
    <div className="relative w-48 h-64 mb-12">
      ...
      <div className="absolute inset-0 bg-gray-50 border-2 border-gray-100 rounded-2xl shadow-sm flex items-center justify-center">
        <FileText className="w-16 h-16 text-gray-200" />
      </div>
      {/* Scanning line that animates from top to bottom, looping */}
      <motion.div
        className="absolute top-0 left-0 right-0 h-1 bg-indigo-500 shadow-[0_0_15px_rgba(79,70,229,0.5)] z-10"
        initial={{ top: "0%" }}
        animate={{ top: "100%" }}
      ...
      <div className="absolute inset-0 flex items-center justify-center opacity-10">
        <div className="grid grid-cols-4 gap-2 w-full px-4">
          {[...Array(12)].map((_, i) => (
            <div key={i} className="h-2 bg-gray-400 rounded-full" />
          ))}
        </div>
      </div>
    </div>
